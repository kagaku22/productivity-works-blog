...
<button class="gp-add-btn" onclick="gpAddCourse()">＋ 科目を追加</button>
...
<button class="gp-reset-btn" onclick="gpReset()">リセット</button>

<!-- freee CTA -->
<div class="gp-freee-cta">
<p>学費・奨学金の管理もクラウド会計で</p>
<span>freee会計なら、収支管理・確定申告をまとめてクラウドで自動化。学生・フリーランス・社会人まで無料で試せます。</span><br>
<a class="gp-freee-btn" href="https://px.a8.net/svt/ejp?a8mat=4B3QAZ+7YYYCY+3SPO+9FHKUP" target="_blank" rel="noopener">freeeを無料で試す &rarr;</a>
</div>

<div class="gp-crosslinks">
パーセント計算 &rarr; <a href="/ja/tools/percentage-calculator/">パーセント計算ツール</a><br>
関数電卓 &rarr; <a href="/ja/tools/scientific-calculator/">関数電卓</a>
</div>
...
ul.innerHTML = '<li style="padding:16px 14px;font-size:13px;color:#94a3b8;text-align:center;">科目がまだ追加されていません。「＋ 科目を追加」をクリックしてください。</li>';
return;
}
ul.innerHTML = GP.courses.map(function(c) {
return '<li class="gp-course-row" id="gp-row-' + c.id + '">' +
'<input class="gp-input" type="text" placeholder="例：数学" value="' + escAttr(c.name) + '" ' +
...
'</select>' +
'<button class="gp-remove-btn" onclick="gpRemoveCourse(' + c.id + ')" title="削除">&#x2715;</button>' +
'</li>';
}).join('');
}
...
document.getElementById('gp-scale-label').textContent = '/ ' + GP.scale.toFixed(1) + ' スケール';
}
...
var grades = currentGrades();
GP.courses.push({ id: GP.nextId++, name: '', credits: 2, grade: grades[1].label });
renderCourseList();
gpRecalcInternal();
...
GP.courses = [
{ id: GP.nextId++, name: '情報科学入門', credits: 2, grade: initGrades[1].label },
{ id: GP.nextId++, name: '微積分学I',    credits: 4, grade: initGrades[4].label },
{ id: GP.nextId++, name: '英語作文',     credits: 2, grade: initGrades[2].label }
];
renderCourseList();
renderScaleRef();
gpRecalcInternal();
