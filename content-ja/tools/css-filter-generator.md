---
title: "CSSフィルタージェネレーター — 画像エフェクトツール"
date: 2025-07-26
description: "ライブプレビューでCSSフィルターエフェクトを作成。ぼかし・明度・コントラスト等を調整。ヴィンテージ・モノクロ等のプリセット。CSSフィルターコードを即コピー。"
categories: ["無料ツール"]
...
<p class="cf-section-title">CSSフィルタージェネレーター</p>
<p class="cf-section-sub">スライダーを動かしてフィルターエフェクトを作成し、CSSをそのままコピーできます。画像をドラッグ＆ドロップするか、URLを貼り付けてご自身の画像でお試しください。</p>
...
<button class="cf-btn cf-btn-sm" id="cf-compare-btn">&#9654; 適用前後を比較</button>
<button class="cf-btn cf-btn-sm cf-btn-primary" id="cf-copy-main-btn">CSSをコピー</button>
</div>
</div>
...
<h3>フィルター設定</h3>
<button class="cf-btn cf-btn-sm cf-btn-danger" id="cf-reset-all-btn">全リセット</button>
</div>
...
/* ── フィルター定義（日本語ラベル） ── */
const FILTERS = [
{ id: 'blur',       label: 'ぼかし',       min: 0,   max: 20,  step: 0.1, def: 0,   unit: 'px',  fmt: v => v === 0 ? null : `blur(${v}px)` },
{ id: 'brightness', label: '明度',         min: 0,   max: 3,   step: 0.01,def: 1,   unit: '',    fmt: v => v === 1 ? null : `brightness(${v})` },
{ id: 'contrast',   label: 'コントラスト', min: 0,   max: 3,   step: 0.01,def: 1,   unit: '',    fmt: v => v === 1 ? null : `contrast(${v})` },
{ id: 'grayscale',  label: 'グレースケール',min: 0,  max: 100, step: 1,   def: 0,   unit: '%',   fmt: v => v === 0 ? null : `grayscale(${v}%)` },
{ id: 'hue-rotate', label: '色相回転',     min: 0,   max: 360, step: 1,   def: 0,   unit: 'deg', fmt: v => v === 0 ? null : `hue-rotate(${v}deg)` },
{ id: 'invert',     label: '階調反転',     min: 0,   max: 100, step: 1,   def: 0,   unit: '%',   fmt: v => v === 0 ? null : `invert(${v}%)` },
{ id: 'opacity',    label: '透明度',       min: 0,   max: 1,   step: 0.01,def: 1,   unit: '',    fmt: v => v === 1 ? null : `opacity(${v})` },
{ id: 'saturate',   label: '彩度',         min: 0,   max: 5,   step: 0.01,def: 1,   unit: '',    fmt: v => v === 1 ? null : `saturate(${v})` },
{ id: 'sepia',      label: 'セピア',       min: 0,   max: 100, step: 1,   def: 0,   unit: '%',   fmt: v => v === 0 ? null : `sepia(${v}%)` },
];
...
/* ── プリセット（日本語名） ── */
const PRESETS = {
'ヴィンテージ':   { blur:0, brightness:1.05, contrast:1.1, grayscale:10, 'hue-rotate':15, invert:0, opacity:1, saturate:0.85, sepia:30, shadow:{ x:0,y:0,blur:0,color:'#000000' } },
'モノクロ':       { blur:0, brightness:0.9,  contrast:1.4, grayscale:100,'hue-rotate':0,  invert:0, opacity:1, saturate:0,    sepia:0,  shadow:{ x:0,y:0,blur:0,color:'#000000' } },
'ウォーム':       { blur:0, brightness:1.1,  contrast:1.05,grayscale:0, 'hue-rotate':20,  invert:0, opacity:1, saturate:1.3,  sepia:15, shadow:{ x:0,y:0,blur:0,color:'#000000' } },
'クール':         { blur:0, brightness:1.05, contrast:1.05,grayscale:0, 'hue-rotate':200, invert:0, opacity:1, saturate:1.2,  sepia:0,  shadow:{ x:0,y:0,blur:0,color:'#000000' } },
'高コントラスト': { blur:0, brightness:1.0,  contrast:2.0, grayscale:0, 'hue-rotate':0,   invert:0, opacity:1, saturate:1.2,  sepia:0,  shadow:{ x:0,y:0,blur:0,color:'#000000' } },
'フェード':       { blur:0, brightness:1.15, contrast:0.75,grayscale:10,'hue-rotate':0,   invert:0, opacity:0.9,saturate:0.7, sepia:10, shadow:{ x:0,y:0,blur:0,color:'#000000' } },
'ドラマチック':   { blur:0, brightness:0.85, contrast:1.6, grayscale:0, 'hue-rotate':0,   invert:0, opacity:1, saturate:1.4,  sepia:0,  shadow:{ x:2,y:2,blur:8,color:'#000000' } },
'クラシックセピア':{ blur:0,brightness:1.05, contrast:1.1, grayscale:0, 'hue-rotate':0,   invert:0, opacity:1, saturate:0.9,  sepia:80, shadow:{ x:0,y:0,blur:0,color:'#000000' } },
};
...
ctx.beginPath(); ctx.arc(480, 80, 80, 0, Math.PI*2); ctx.fill();
...
ctx.beginPath(); ctx.arc(cx-s, cy+s*0.3, s*0.7, 0, Math.PI*2); ctx.fill();
...
ctx.fillStyle = color;
ctx.beginPath();
ctx.moveTo(x-w/2, 260); ctx.lineTo(x, 260-h); ctx.lineTo(x+w/2, 260);
ctx.closePath(); ctx.fill();
}
...
for(let i=0;i<25;i++) {
const fx = 30+Math.random()*540, fy = 270+Math.random()*110;
...
ctx.fillStyle = water;
ctx.beginPath(); ctx.ellipse(300, 355, 90, 32, 0, 0, Math.PI*2); ctx.fill();
ctx.strokeStyle = 'rgba(255,255,255,0.4)';
ctx.lineWidth = 1.5;
for(let i=0;i<5;i++) {
ctx.beginPath(); ctx.moveTo(230+i*14, 350+i%2*4); ctx.lineTo(255+i*14, 350+i%2*4); ctx.stroke();
}
...
<span class="cf-filter-name">ドロップシャドウ</span>
<button class="cf-reset-single" id="cf-reset-shadow" title="リセット">↺</button>
</div>
<div class="cf-shadow-sub">
<div>
<label>X オフセット (px)</label>
...
compareBtn.textContent = compareMode ? '✕ 比較終了' : '▶ 適用前後を比較';
});
...
/* ── CSSコピー ── */
function copyToClipboard(text, btnEl, label) {
navigator.clipboard.writeText(text).then(() => {
btnEl.classList.add('cf-copied');
btnEl.textContent = 'コピー完了!';
setTimeout(() => {
btnEl.classList.remove('cf-copied');
btnEl.textContent = label;
}, 1800);
showToast('CSSをクリップボードにコピーしました！');
}).catch(() => {
...
copyToClipboard(`filter: ${buildFilterString()};`, copyMainBtn, 'CSSをコピー');
});
copyOutBtn.addEventListener('click', () => {
copyToClipboard(`filter: ${buildFilterString()};`, copyOutBtn, 'コピー');
});
...
> 色のコントラスト確認 → [カラーコントラストチェッカー](/ja/tools/color-contrast-checker/)
> 画像をリサイズ → [画像リサイザー](/ja/tools/image-resizer/)

<div class="cf-freee-cta">
<div class="cf-freee-icon">&#128196;</div>
<div class="cf-freee-body">
