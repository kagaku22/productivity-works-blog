...
title: "Markdownエディター"
date: 2025-12-06
description: "無料のオンラインMarkdownエディター。リアルタイムプレビュー・ツールバー・HTML出力・.mdダウンロード対応。記事執筆・ドキュメント作成に最適。"
categories: ["無料ツール"]
tags: ["マークダウンエディター", "markdown", "リアルタイムプレビュー", "wysiwyg", "ライティングツール", "開発ツール"]
slug: "markdown-editor"
cover:
  image: "/images/covers/markdown-editor-ja.png"
  alt: "Markdownエディター リアルタイムプレビュー"
ShowToc: false
---
...
var readMin = Math.max(1, Math.ceil(words / 400));
...
document.getElementById('me-stat-read').textContent = readMin + ' 分';
}
...
switch (type) {
case 'bold':      wrapSelection('**', '**', '太字テキスト'); break;
case 'italic':    wrapSelection('*', '*', '斜体テキスト'); break;
case 'strike':    wrapSelection('~~', '~~', '取り消し線'); break;
case 'code':      wrapSelection('`', '`', 'コード'); break;
...
case 'link':
var url = window.prompt('URLを入力:', 'https://');
if (url) wrapSelection('[', '](' + url + ')', 'リンクテキスト');
break;
case 'image':
var imgUrl = window.prompt('画像URLを入力:', 'https://');
var altText = imgUrl ? (window.prompt('代替テキストを入力:', '画像') || '画像') : null;
if (imgUrl) insertBlock('![' + altText + '](' + imgUrl + ')');
break;
case 'codeblock':
var lang = window.prompt('言語を入力（例: javascript）:', '') || '';
insertBlock('```' + lang + '\n' + (getEditor().value.substring(getEditor().selectionStart, getEditor().selectionEnd) || 'ここにコードを入力') + '\n```');
break;
case 'table':
...
darkMode = !darkMode;
document.getElementById('me-app').classList.toggle('me-dark', darkMode);
};
...
msg.textContent = 'HTMLをコピーしました！';
msg.classList.add('me-show');
setTimeout(function() { msg.textContent = '\u2713 自動保存済み'; msg.classList.remove('me-show'); }, 2000);
});
} else {
...
msg.textContent = 'HTMLをコピーしました！'; msg.classList.add('me-show');
setTimeout(function() { msg.textContent = '\u2713 自動保存済み'; msg.classList.remove('me-show'); }, 2000);
}
};
...
if (window.confirm('内容をすべてクリアしますか？')) {
