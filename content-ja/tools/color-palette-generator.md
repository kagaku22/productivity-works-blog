...
<div class="pa-wrap">
<p class="pa-lead">ベースカラーを選んでハーモニーモードを切り替えるだけで、色彩理論に基づいた美しい5色パレットを瞬時に生成します。色をロックして他の色だけを再生成したり、HEXコードをワンクリックでコピーできます。</p>
...
<div class="pa-controls">
  ...
  <div class="pa-group">
    ...
    <select id="pa-harmony" class="pa-select">
      ...
      <option value="triadic">トライアド (Triadic)</option>
      <option value="split-complementary">分割補色 (Split-Complementary)</option>
      <option value="monochromatic">モノクロマティック</option>
    </select>
  </div>
  <button class="pa-btn pa-btn-primary" id="pa-generate-btn">生成する</button>
  <button class="pa-btn pa-btn-random" id="pa-random-btn">&#9684; ランダム</button>
</div>
...
<div class="pa-export">
  <div class="pa-export-header">
    <h3>CSS変数エクスポート</h3>
    <button class="pa-btn pa-btn-secondary" id="pa-copy-css-btn">CSSをコピー</button>
  </div>
  <div class="pa-export-box" id="pa-css-output"></div>
</div>

<hr class="pa-divider">
<p class="pa-presets-title">トレンドパレット プリセット10選</p>
<div class="pa-presets" id="pa-presets"></div>

<div class="pa-toast" id="pa-toast"></div>

<div class="pa-cta">
  <div class="pa-cta-text">
...
(function(){
  ...
  // ── プリセット ───────────────────────────────────────────────────────────
  const PRESETS=[
    {name:'ネオンサンセット', colors:['#ff6b6b','#ffa36c','#ffd93d','#6bcb77','#4d96ff']},
    {name:'深海ブルー',       colors:['#03045e','#0077b6','#00b4d8','#90e0ef','#caf0f8']},
    {name:'森の静寂',         colors:['#1b4332','#2d6a4f','#52b788','#b7e4c7','#d8f3dc']},
    {name:'ベリーブリス',     colors:['#6a0572','#ab83a1','#d6a2e8','#f8a5c2','#ff6b9d']},
    {name:'砂と石',           colors:['#3d2b1f','#795548','#a1887f','#d7ccc8','#efebe9']},
    {name:'北欧チル',         colors:['#2b2d42','#8d99ae','#edf2f4','#ef233c','#d90429']},
    {name:'柑橘バースト',     colors:['#ff4800','#ff6d00','#ff9e00','#ffbd00','#ffd500']},
    {name:'ラベンダーフォグ', colors:['#7b2d8b','#9b5de5','#c77dff','#e0aaff','#f3d9fa']},
    {name:'ミッドナイト',     colors:['#0d0d0d','#1a1a2e','#16213e','#0f3460','#533483']},
    {name:'春の新緑',         colors:['#2dc653','#80ed99','#c7f9cc','#ffe5d9','#ff9a76']},
  ];
  ...
  function render(){
    ...
    palette.forEach((c,i)=>{
      ...
      sw.title=c+'をコピー';
      sw.onclick=()=>{navigator.clipboard.writeText(c).then(()=>showToast(c+' をコピーしました'));};

      const lockBtn=document.createElement('button');
      lockBtn.className='pa-lock-btn'+(locked[i]?' locked':'');
      lockBtn.title=locked[i]?'ロック解除':'色をロック';
  ...
  document.getElementById('pa-copy-css-btn').addEventListener('click',()=>{
    navigator.clipboard.writeText(cssOutput.textContent).then(()=>showToast('CSSをコピーしました'));
  });
