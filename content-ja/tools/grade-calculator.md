...
<h2>成績計算ツール</h2>
<p class="gc-subtitle">課題ごとの配点比率（ウェイト）を考慮した加重平均・成績グレード・GPAをリアルタイムで計算。期末試験で必要なスコアも一発表示。</p>

<!-- 課題入力カード -->
<div class="gc-card">
<div class="gc-card-title"><span class="icon">📋</span> 課題・試験の成績入力</div>
<table class="gc-table" id="gc-table">
<thead>
<tr>
<th style="width:30%">課題名</th>
<th style="width:15%">得点</th>
<th style="width:15%">満点</th>
<th style="width:15%">配点比率(%)</th>
<th style="width:12%">成績</th>
<th style="width:10%">得点率</th>
<th style="width:3%"></th>
</tr>
</thead>
<tbody id="gc-tbody">
...
<button class="gc-btn gc-btn-primary" onclick="gcAddRow()">+ 課題を追加</button>
<button class="gc-btn gc-btn-secondary" onclick="gcReset()">リセット</button>
</div>
...
<div class="gc-weight-label" id="gc-weight-label" style="color:#6366f1;">合計: 0%</div>
</div>
...
<!-- 期末試験逆算カード -->
<div class="gc-card">
<div class="gc-card-title"><span class="icon">🎯</span> 期末試験で必要なスコアを逆算</div>
<div class="gc-final-grid">
<div>
<label style="font-size:0.875rem;font-weight:600;color:#475569;">期末試験の配点比率（%）</label>
<div class="gc-final-input-row">
<input type="number" id="gc-final-weight" min="0" max="100" value="30" oninput="gcCalcFinal()" placeholder="例: 30">
<span style="font-size:0.85rem;color:#666;">% の配点</span>
</div>
</div>
<div>
<label style="font-size:0.875rem;font-weight:600;color:#475569;">目標の最終成績</label>
<div class="gc-final-input-row">
...
<!-- freee CTA -->
<div class="gc-cta">
<strong>学費・教育費の管理に</strong><br>
奨学金・授業料・教材費などの教育費を自動で帳簿管理。確定申告の教育費控除もスムーズに。<br>
学費管理を効率化 → <a href="https://px.a8.net/svt/ejp?a8mat=4B3QAZ+7YYYCY+3SPO+9FHKUP" target="_blank" rel="noopener">freee会計で教育費を自動管理</a>
</div>
...
if (gcRows.length === 0) {
tbody.innerHTML = '<tr class="gc-empty-row"><td colspan="7">課題がありません。「課題を追加」ボタンで追加してください。</td></tr>';
return;
}
tbody.innerHTML = gcRows.map(function(r) {
return '<tr id="gc-row-' + r.id + '">' +
'<td><input type="text" id="gc-name-' + r.id + '" placeholder="例: 中間試験" oninput="gcUpdate()"></td>' +
...
'<td><button class="gc-btn-remove" onclick="gcRemoveRow(' + r.id + ')" title="削除">&#x2715;</button></td>' +
'</tr>';
}).join("");
}
...
wlabel.textContent = "合計: 0%";
alertEl.innerHTML = "";
} else if (Math.abs(totalWeight - 100) < 0.01) {
fill.style.background = "#16a34a";
wlabel.style.color = "#16a34a";
wlabel.textContent = "合計: 100% \u2713";
alertEl.innerHTML = '<div class="gc-alert gc-alert-ok">配点比率の合計が100%です。加重平均は正確に計算されています。</div>';
} else if (totalWeight > 100) {
fill.style.background = "#ef4444";
wlabel.style.color = "#ef4444";
wlabel.textContent = "合計: " + totalWeight.toFixed(1) + "% \u26A0";
alertEl.innerHTML = '<div class="gc-alert gc-alert-err">配点比率が100%を' + (totalWeight - 100).toFixed(1) + '%超過しています。修正してください。</div>';
} else {
fill.style.background = "#f59e0b";
wlabel.style.color = "#d97706";
wlabel.textContent = "合計: " + totalWeight.toFixed(1) + "%";
alertEl.innerHTML = '<div class="gc-alert gc-alert-warn">配点比率の合計が' + totalWeight.toFixed(1) + '%です。残り' + (100 - totalWeight).toFixed(1) + '%を設定すると正確な加重平均が計算できます。</div>';
}
...
letterSub.textContent = "配点比率未設定（素点平均）";
...
var currentCls = hasAny && totalWeight > 0
? getGradeInfo((weightedSum / totalWeight) * 100).cls
: (totalMax > 0 ? getGradeInfo((totalScore / totalMax) * 100).cls : null);
chipWrap.innerHTML = GRADES.map(function(g) {
var active = currentCls && g.cls === currentCls ? " active" : "";
...
if (currentAvg === null) {
resultEl.innerHTML = "上の表に得点と配点比率を入力すると、期末試験で必要なスコアが表示されます。";
return;
}
...
if (neededPct > 100) {
resultEl.innerHTML = '目標成績 <strong>' + targetGi.label + '（' + targetEl.value + '%）</strong> を達成するには、期末試験で <strong>' + neededPct.toFixed(1) + '%</strong> が必要です。100%を超えているため、目標グレードを下げるか、教授に追加課題を相談してみてください。';
} else if (neededPct < 0) {
resultEl.innerHTML = 'おめでとうございます！現時点で <strong>' + targetGi.label + '</strong> は確定しています。期末試験が0点でも目標グレードを達成できます。';
} else {
var neededGi = getGradeInfo(neededPct);
resultEl.innerHTML = '目標成績 <strong>' + targetGi.label + '（' + targetEl.value + '%）</strong> を達成するには、期末試験（配点' + (finalWeight * 100).toFixed(0) + '%）で <strong>' + neededPct.toFixed(1) + '% （<span class="gc-grade-badge grade-' + neededGi.cls + '">' + neededGi.label + '</span>）</strong> 以上が必要です。';
}
}
...
// サンプルデータ
gcAddRow("宿題・課題平均", 88, 100, 20);
gcAddRow("中間試験", 76, 100, 25);
gcAddRow("レポート", 92, 100, 15);
gcAddRow("小テスト", 84, 100, 10);
})();
</script>
</div>
