...
ShowToc: false
cover:
  image: "/images/covers/ip-subnet-calculator-ja.png"
  alt: "IPサブネット計算ツール"
---

IPv4アドレスとCIDRプレフィックス（またはサブネットマスク）を入力するだけで、ネットワークアドレス・ブロードキャスト・ホスト範囲・ワイルドカードマスクなどをすぐに計算できます。
...
<p style="margin:0;font-size:14px;color:#0369a1;font-weight:600;">ネットワーク管理の経費もかんたんに</p>
<span style="font-size:13px;color:#0c4a6e;">freee会計なら、サーバー・ネットワーク費用の経費精算もクラウドで一元管理。無料トライアル実施中。</span>
...
> IPアドレスを確認 → [IPアドレス情報ツール](/ja/tools/ip-address-info/)
> バイトを変換 → [バイト変換ツール](/ja/tools/byte-converter/)
> DNSレコード → [DNSレコードガイド](/ja/tools/dns-record-guide/)
