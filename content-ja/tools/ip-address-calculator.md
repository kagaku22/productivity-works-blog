...
<h1>IPアドレス計算ツール</h1>
<p class="ipc-subtitle">サブネット・CIDR・ネットワーク情報をブラウザ上で即時計算。サーバー不要・完全無料。</p>

<div class="ipc-card">
  <h2>IPアドレス / CIDR 入力</h2>
  <div class="ipc-input-row">
    <input id="ipc-input" class="ipc-input" type="text" placeholder="例: 192.168.1.0/24" spellcheck="false" autocomplete="off" />
    <button class="ipc-btn" onclick="ipcCalculate()">計算する</button>
    <button class="ipc-btn" style="background:#6c757d;" onclick="ipcReset()">リセット</button>
  </div>
  <div class="ipc-examples">
    <span style="font-size:0.82rem;color:#666;">例:</span>
...
<div id="ipc-results" class="ipc-results">
  ...
  <div class="ipc-card">
    <h2>サブネット分割計算</h2>
    <p style="font-size:0.88rem;color:#556;margin:0 0 0.8rem 0;">現在のネットワークを、より小さなサブネットに分割します。新しいプレフィックス長を選択してください。</p>
    <div class="ipc-sub-form">
      <label style="font-size:0.9rem;font-weight:600;color:#3a5080;">新しいプレフィックス:</label>
      <select id="ipc-sub-select" class="ipc-select"></select>
      <button class="ipc-btn ipc-btn-sm" onclick="ipcSubdivide()">分割する</button>
    </div>
    <div id="ipc-sub-results"></div>
  </div>

  <div class="ipc-card">
    <h2>CIDR → サブネットマスク 対応表</h2>
...
<div class="ipc-cta">
  <div class="ipc-cta-label">IT管理を効率化</div>
  <p>IT経費・ソフトウェア費用の管理に手間がかかっていませんか？ <a href="https://px.a8.net/svt/ejp?a8mat=4B3QAZ+7YYYCY+3SPO+9FHKUP" target="_blank" rel="noopener">freee会計でIT経費を自動管理</a> すれば、請求書の仕訳・経費計上が自動化できます。</p>
</div>
...
(function() {
  ...
  function ipcParseIP(str) {
    ...
    var out = parts.map(function(p) {
      var n = Number(p);
      if (isNaN(n) || n < 0 || n > 255 || p.trim() === '') { ok = false; }
      return n;
    });
    return ok ? out : null;
  }
  ...
  window.ipcCalculate = function() {
    ...
    var parts = raw.split('/');
    if (parts.length !== 2) {
      errEl.textContent = 'IPアドレスとCIDRプレフィックスをスラッシュで区切って入力してください。例: 192.168.1.0/24';
    ...
    if (!octets) {
      errEl.textContent = 'IPアドレスが不正です。各オクテットは0〜255の整数を入力してください。';
      errEl.style.display = 'block';
      document.getElementById('ipc-results').style.display = 'none';
      return;
    }
    if (isNaN(prefix) || prefix < 0 || prefix > 32) {
      errEl.textContent = 'プレフィックス長が不正です。0〜32の整数を入力してください。';
      errEl.style.display = 'block';
      document.getElementById('ipc-results').style.display = 'none';
      return;
    }
    ...
    // クラス名の日本語説明
    var classDesc = { A: 'クラスA', B: 'クラスB', C: 'クラスC', D: 'クラスD (マルチキャスト)', E: 'クラスE (予約済み)' };

    // バッジ
    var badgeRow = document.getElementById('ipc-badges');
    var classBadgeClass = 'badge-class-' + ipClass.toLowerCase();
    badgeRow.innerHTML =
      '<span class="ipc-badge ' + classBadgeClass + '">' + classDesc[ipClass] + '</span>' +
      '<span class="ipc-badge ' + (priv ? 'badge-private' : 'badge-public') + '">' + (priv ? 'プライベート' : 'パブリック') + '</span>' +
      '<span class="ipc-badge" style="background:#e8eaf6;color:#3a3a6a;border:1px solid #b0b8d8;">/' + prefix + '</span>' +
      (octets[0] === 127 ? '<span class="ipc-badge" style="background:#fce4ec;color:#880e4f;border:1px solid #f48fb1;">ループバック</span>' : '') +
      (octets[0] === 169 && octets[1] === 254 ? '<span class="ipc-badge" style="background:#fff3e0;color:#e65100;border:1px solid #ffcc80;">リンクローカル</span>' : '') +
    ...
    mainFields.innerHTML = [
      { label: 'IPアドレス', value: intToIP(ipInt), sub: intToBinary(ipInt) },
      { label: 'ネットワークアドレス', value: intToIP(netInt), sub: intToBinary(netInt) },
      { label: 'ブロードキャストアドレス', value: intToIP(bcastInt), sub: intToBinary(bcastInt) },
      { label: '最初の使用可能ホスト', value: intToIP(firstHost), sub: '' },
      { label: '最後の使用可能ホスト', value: intToIP(lastHost), sub: '' },
      { label: '総アドレス数', value: formatNum(totalHosts), sub: 'ネットワーク・ブロードキャスト含む' },
      { label: '使用可能ホスト数', value: formatNum(usableHosts >= 0 ? usableHosts : 0), sub: '割り当て可能なIPアドレス数' },
    ].map(function(f) {
    ...
    maskFields.innerHTML = [
      { label: 'サブネットマスク (10進数)', value: intToIP(maskInt), sub: intToBinary(maskInt) },
      { label: 'ワイルドカードマスク', value: intToIP(wildInt), sub: intToBinary(wildInt) },
      { label: 'CIDR表記', value: '/' + prefix, sub: 'ネットワークビット: ' + prefix + '、ホストビット: ' + (32 - prefix) },
      { label: 'ネットワーク整数値', value: netInt.toString(), sub: '0x' + netInt.toString(16).toUpperCase().padStart(8, '0') },
    ].map(function(f) {
      return '<div class="ipc-field"><div class="ipc-field-label">' + f.label + '</div>' +
        '<div class="ipc-field-value">' + f.value + '</div>' +
        (f.sub ? '<div class="ipc-field-sub">' + f.sub + '</div>' : '') + '</div>';
    ...
    for (var p = prefix + 1; p <= 30; p++) {
      ...
      opt.value = p;
      opt.textContent = '/' + p + ' (各サブネット ' + formatNum(Math.pow(2, 32 - p) - 2) + ' ホスト)';
      sel.appendChild(opt);
    }
  ...
  window.ipcSubdivide = function() {
    ...
    var extra = count > display ? '<p style="font-size:0.82rem;color:#888;margin:0.4rem 0 0 0;">' + formatNum(count) + ' サブネット中、最初の ' + display + ' 件を表示しています。</p>' : '';
    document.getElementById('ipc-sub-results').innerHTML =
      '<div style="overflow-x:auto;"><table class="ipc-table"><thead><tr><th>#</th><th>サブネット</th><th>ネットワーク</th><th>ブロードキャスト</th><th>最初のホスト</th><th>最後のホスト</th><th>使用可能数</th></tr></thead><tbody>' +
