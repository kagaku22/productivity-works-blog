...
(function() {
  ...
  function jpEval(obj, path) {
    path = path.trim();
    if (!path.startsWith('$')) throw new Error('パスは $ で始める必要があります');

    var tokens = tokenize(path.slice(1));
      ...
      nodes.forEach(function(node) {
        ...
        if (rest.length > 0 && rest[0].type === 'key') {
          ...
          descendants.forEach(function(d) {
            ...
            if (r.length > 0) {
              results = results.concat(query(r, restRest));
            }
          });
        } else {
          descendants.forEach(function(d) {
            results = results.concat(query([d], rest));
          });
        }
      });
      return results;
  ...
  window.jpRun = function() {
    ...
    if (!jsonStr) {
      resultArea.innerHTML = '<div class="placeholder-msg">JSONとJSONPath式を入力すると結果が表示されます</div>';
      return;
    }
    try {
      data = JSON.parse(jsonStr);
    } catch (e) {
      jsonErrEl.textContent = 'JSON解析エラー: ' + e.message;
      jsonInp.classList.add('error');
      resultArea.innerHTML = '<div class="error-panel">JSONエラー: ' + escHtml(e.message) + '</div>';
      return;
    }

    if (!pathStr) {
      resultArea.innerHTML = '<div class="placeholder-msg">JSONPath式を入力してください</div>';
      return;
    }
    ...
    try {
      ...
      pathErrEl.textContent = 'パスエラー: ' + e.message;
      pathInp.classList.add('error');
      resultArea.innerHTML = '<div class="error-panel">パスエラー: ' + escHtml(e.message) + '</div>';
      return;
    }
    ...
    resultArea.innerHTML =
      '<div class="result-panel">' +
        '<div class="result-header">' +
          '<span class="result-count"><span class="match-num">' + count + '</span> 件マッチ</span>' +
          (count > 0 ? '<button class="copy-result-btn" id="jp-copy-btn" onclick="jpCopyResult()">結果をコピー</button>' : '') +
        '</div>' +
        (count === 0
          ? '<div class="placeholder-msg" style="padding:16px 0;">マッチするデータが見つかりませんでした</div>'
    ...
    if (!ta.value) {
      ta.value = JSON.stringify({
        "store": {
          "name": "本の世界",
          "book": [
            { "title": "世紀の格言", "author": "作者A", "price": 8.95 },
            { "title": "名誉の剣", "author": "作者B", "price": 12.99 },
            { "title": "白鯨", "author": "作者C", "price": 8.99 },
            { "title": "指輪物語", "author": "作者D", "price": 22.99 }
          ],
          "bicycle": { "color": "赤", "price": 19.95 }
        }
      }, null, 2);
    }
  });
