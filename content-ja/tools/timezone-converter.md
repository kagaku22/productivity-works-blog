---
title: "タイムゾーン変換ツール - 無料ワールドクロック・会議時間調整"
date: 2025-09-28
description: "ライブ世界時計、日時変換、複数タイムゾーン比較、会議プランナー機能を備えた無料オンラインタイムゾーン変換ツール。20以上の主要タイムゾーンを瞬時に変換。"
categories: ["無料ツール"]
...
<div id="tz-section-meeting" class="tz-section">
  <div class="tz-card">
    ...
    <p style="font-size:13px;color:#6b7280;margin:0 0 16px;">選択した各タイムゾーンで9時〜17時（業務時間）が重なる時間帯を表示します。まずワールドクロックタブでタイムゾーンを追加してください。</p>
    <div class="tz-meeting-grid">
      <table class="tz-meeting-table" id="tz-meeting-table"></table>
    </div>
    <div class="tz-legend">
      <div class="tz-legend-item"><div class="tz-legend-dot" style="background:#1e1b4b"></div>夜間</div>
      <div class="tz-legend-item"><div class="tz-legend-dot" style="background:#fef9c3;border:1px solid #d1d5db"></div>昼間（業務時間外）</div>
      <div class="tz-legend-item"><div class="tz-legend-dot" style="background:#6ee7b7"></div>業務時間</div>
      <div class="tz-legend-item"><div class="tz-legend-dot" style="background:#4f46e5"></div>全員の重複時間</div>
    </div>
  </div>
</div>
...
(function() {
  const ZONES = [
    { label: "JST — 東京（日本標準時）",              value: "Asia/Tokyo" },
    { label: "UTC — 協定世界時",                       value: "UTC" },
    { label: "EST — ニューヨーク",                     value: "America/New_York" },
    { label: "CST — シカゴ",                           value: "America/Chicago" },
    { label: "MST — デンバー",                         value: "America/Denver" },
    { label: "PST — ロサンゼルス",                     value: "America/Los_Angeles" },
    { label: "AKT — アラスカ",                         value: "America/Anchorage" },
    { label: "HST — ハワイ",                           value: "Pacific/Honolulu" },
    { label: "BRT — サンパウロ",                       value: "America/Sao_Paulo" },
    { label: "GMT — ロンドン",                         value: "Europe/London" },
    { label: "CET — パリ・ベルリン",                   value: "Europe/Paris" },
    { label: "EET — ヘルシンキ・アテネ",               value: "Europe/Helsinki" },
    { label: "MSK — モスクワ",                         value: "Europe/Moscow" },
    { label: "GST — ドバイ",                           value: "Asia/Dubai" },
    { label: "IST — ムンバイ・デリー",                 value: "Asia/Kolkata" },
    { label: "BST — ダッカ",                           value: "Asia/Dhaka" },
    { label: "ICT — バンコク",                         value: "Asia/Bangkok" },
    { label: "SGT — シンガポール",                     value: "Asia/Singapore" },
    { label: "CST — 上海・北京",                       value: "Asia/Shanghai" },
    { label: "KST — ソウル",                           value: "Asia/Seoul" },
    { label: "AEST — シドニー",                        value: "Australia/Sydney" },
    { label: "NZST — オークランド",                    value: "Pacific/Auckland" },
  ];
  ...
  function init() {
    populateSelect('tz-from', 'Asia/Tokyo');
    populateSelect('tz-to', 'America/New_York');
    populateSelect('tz-add-select', 'UTC');
  ...
  function tzConvert() {
    ...
    document.getElementById('tz-conv-offset').textContent =
      '時差: ' + sign + diffH.toFixed(1).replace('.0','') + '時間';
  }
  ...
  function formatInTz(utcMs, tz) {
    return new Date(utcMs).toLocaleString('ja-JP', {
      timeZone: tz,
  ...
  function formatDateOnly(utcMs, tz) {
    return new Date(utcMs).toLocaleDateString('ja-JP', {
  ...
  function tickClock() {
    ...
    const items = document.querySelectorAll('#tz-app .tz-multi-item');
    items.forEach(item => {
  ...
  function renderMultiList() {
    ...
    multiZones.forEach(tz => {
      ...
      item.innerHTML = `
        ...
        <span class="tz-multi-badge ${isDay ? 'badge-day':'badge-night'}">${isDay?'昼':'夜'}</span>
  ...
  function renderTimeline(now) {
    ...
    let html = '<div class="tz-tl-label">24時間タイムライン（黄色 = 午前6時〜午後6時）</div>';
    multiZones.forEach(tz => {
      const z = ZONES.find(z => z.value === tz);
  ...
  function tzRenderMeeting() {
    ...
    if (multiZones.length === 0) {
      table.innerHTML = '<tr><td style="padding:20px;color:#9ca3af;font-size:13px">ワールドクロックタブでタイムゾーンを追加してください。</td></tr>';
      return;
    }
    const now = Date.now();
    let html = '<thead><tr><th class="tz-row-label">タイムゾーン</th>';
    for (let h = 0; h < 24; h++) {
      html += `<th>${h}</th>`;
    }
    html += '</tr></thead><tbody>';
    ...
    multiZones.forEach((tz, zi) => {
      ...
      for (let h = 0; h < 24; h++) {
        ...
        html += `<td class="${cls}" title="${localH}時"></td>`;
      }
      html += '</tr>';
    });
...
**海外取引の会計管理にはfreee**
海外クライアントとの取引がある方、freeeなら外貨取引も簡単に記帳できます。
