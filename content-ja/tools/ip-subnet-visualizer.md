---
title: "IPサブネット可視化ツール - 無料CIDRサブネット計算"
date: 2025-05-16
description: "視覚的なCIDRサブネット計算ツール。IP/CIDRを入力するだけで、ネットワークアドレス・ブロードキャスト・使用可能ホスト範囲・ホスト数を即座に表示。2進数カラー表示・サブネット一覧表・アドレス空間の可視化付き。"
categories: ["無料ツール"]
tags: ["サブネット計算", "CIDR計算", "IPサブネット", "ネットワークアドレス", "ブロードキャストアドレス", "サブネットマスク", "IPv4"]
slug: "ip-subnet-visualizer"
aliases: ["/ja/tools/ip-subnet-visualizer/"]
cover:
  image: "/images/covers/ip-subnet-visualizer-ja.png"
  alt: "IPサブネット可視化ツール"
ShowToc: false
---
...
(function() {
  ...
  window.isvCalc = function() {
    ...
    if (!parsed) {
      errEl.textContent = '無効なCIDR表記です。形式: 192.168.1.0/24';
    ...
    var cards = [
      { label: 'IPアドレス', value: parsed.ipStr, cls: '' },
      { label: 'ネットワークアドレス', value: numToIp(networkNum), cls: '' },
      { label: 'ブロードキャスト', value: numToIp(broadcastNum), cls: 'orange' },
      { label: 'サブネットマスク', value: numToIp(mask), cls: '' },
      { label: '最初のホスト', value: prefix < 31 ? numToIp(firstHost) : 'N/A', cls: 'green' },
      { label: '最後のホスト', value: prefix < 31 ? numToIp(lastHost) : 'N/A', cls: 'green' },
      { label: '使用可能ホスト数', value: hostCount > 0 ? hostCount.toLocaleString() : (prefix === 32 ? '1' : '0'), cls: 'yellow' },
      { label: '総アドレス数', value: totalAddrs.toLocaleString(), cls: '' },
    ];
    ...
    var binHtml = '';
    binHtml += renderBinaryRow('IPアドレス', ipNum, prefix, 'ip');
    binHtml += renderBinaryRow('サブネットマスク', mask, prefix, 'mask');
    binHtml += renderBinaryRow('ネットワーク', networkNum, prefix, 'ip');
    binHtml += renderBinaryRow('ブロードキャスト', broadcastNum, prefix, 'ip');
  ...
  window.addEventListener('load', function() {
    ...
    window.addEventListener('resize', function() {
      if (document.getElementById('isv-input').value.trim()) isvCalc();
    });
  });
})();
</script>
...
> IPアドレス情報 → [IPアドレス情報ツール](/ja/tools/ip-address-info/)
