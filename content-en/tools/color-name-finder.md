...
description: "Free color name finder tool. Enter any hex, RGB, or HSL color and get the closest named CSS color. Browse 148 named CSS colors with search and copy features."
date: 2025-05-16
slug: color-name-finder
categories: ["Free Tools"]
tags: ["color", "css", "design", "web tools"]
cover:
  image: /images/covers/color-name-finder.png
  alt: "Color Name Finder Tool"
ShowToc: false
---

Find the closest CSS named color to any hex, RGB, or HSL value — instantly. Browse all 148 CSS named colors, search by name, and copy values with one click.
...
<div class="cn-card">
  ...
  <div id="cn-sec-hex" class="cn-section active">
    ...
    <p class="cn-notice">Enter a 6-digit hex code (e.g. #ff6347)</p>
  </div>
  ...
  <div id="cn-sec-rgb" class="cn-section">
    ...
    <div style="margin-top:12px;">
      <button class="cn-btn" onclick="cnApp.findFromRgb()">Find Name</button>
    </div>
  </div>
  ...
  <div id="cn-sec-pick" class="cn-section">
    <div class="cn-row">
      ...
      <span style="font-size:13px;color:#64748b;align-self:center;">Pick any color to find its CSS name</span>
    </div>
  </div>
</div>
  ...
  <div class="cn-swatch-row">
    <div class="cn-swatch-box">
      ...
      <div class="cn-swatch-info">
        <div class="cn-swatch-label">Your Color</div>
    ...
    <div class="cn-swatch-box">
      ...
      <div class="cn-swatch-info">
        <div class="cn-swatch-label">Closest CSS Name</div>
...
<!-- Browse Section -->
<div class="cn-card">
  <h2>Browse All 148 CSS Named Colors</h2>
  <div class="cn-search-row">
    <div class="cn-field">
      <label>Search</label>
      <input type="text" id="cn-search" placeholder="e.g. blue, red..." oninput="cnApp.filterColors()">
    </div>
  </div>
...
<div class="cn-modal-overlay" id="cn-modal">
  <div class="cn-modal">
    ...
    <button class="cn-btn" style="margin-top:14px;width:100%;" onclick="cnApp.useModalColor()">Use This Color</button>
  </div>
</div>
...
(function() {
  ...
  var FAMILIES = ['all','reds','oranges','yellows','greens','blues','purples','pinks','grays','whites'];
  ...
  function buildValRows(hex, r, g, b) {
    ...
    return [
      ...
      return '<div class="cn-val-row"><span class="cn-val-label">'+item.label+'</span><span class="cn-val-text">'+item.val+'</span><button class="cn-copy-icon" onclick="navigator.clipboard.writeText(\''+item.val.replace(/'/g,"\\'")+'\').then(function(){var b=this;}).call(this);" onclick="cnApp.cp(\''+item.val.replace(/'/g,"\\'")+'\',this)">copy</button></div>';
    }).join('');
  }
  ...
  function showResult(inputHex, inputR, inputG, inputB) {
    ...
    function makeRows(vals) {
      return vals.map(function(item){
        ...
        return '<div class="cn-val-row"><span class="cn-val-label">'+item.label+'</span><span class="cn-val-text">'+item.val+'</span><button class="cn-copy-icon" onclick="navigator.clipboard.writeText(this.previousElementSibling.textContent)">copy</button></div>';
      }).join('');
    }
    ...
    document.getElementById('cn-distance-info').innerHTML =
      'Color distance: <span>'+match.distance.toFixed(1)+'</span> (Euclidean RGB) — Match accuracy: <span>'+Math.max(0,pct)+'%</span>';
  ...
  function buildGrid(list) {
    ...
    document.getElementById('cn-count').textContent = list.length + ' colors shown';
  }
  ...
  function showModal(c) {
    ...
    document.getElementById('cn-modal-vals').innerHTML = vals.map(function(item){
      return '<div class="cn-modal-val-row"><span class="cn-modal-val-label">'+item.label+'</span><span class="cn-modal-val-text">'+item.val+'</span><button class="cn-modal-copy" onclick="navigator.clipboard.writeText(this.previousElementSibling.textContent)">Copy</button></div>';
    }).join('');
  ...
  function buildFamilyFilters() {
    ...
    FAMILIES.forEach(function(f){
      ...
      btn.className = 'cn-filter-btn' + (f==='all'?' active':'');
  ...
  window.cnApp = {
    findFromHex: function() {
      ...
      if(!val) { alert('Please enter a valid hex color (e.g. #ff6347)'); return; }
...
- Check contrast → [Color Contrast Checker](/tools/color-contrast-checker/)
