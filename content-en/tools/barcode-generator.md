...
description: "Free online barcode generator. Create Code 128, Code 39, EAN-13, UPC-A, and ITF barcodes. Download as PNG or SVG. No sign-up required, fully client-side."
...
cover:
  ...
  alt: "Barcode Generator - Free online tool to create Code 128, Code 39, and EAN-13 barcodes"
---
...
<label class="bg-label" for="bg-data">Data / Value</label>
<div class="bg-samples" id="bg-samples"></div>
<input id="bg-data" class="bg-input" type="text" placeholder="Enter barcode data..." />

<div class="bg-row">
<div class="bg-col">
<label class="bg-label" for="bg-bar-color">Bar Color</label>
<input id="bg-bar-color" class="bg-color" type="color" value="#000000" />
</div>
<div class="bg-col">
<label class="bg-label" for="bg-bg-color">Background Color</label>
...
<div class="bg-row">
<div class="bg-col">
<label class="bg-label">Bar Width: <span id="bg-bw-val">2</span>px</label>
<input id="bg-bar-width" class="bg-range" type="range" min="1" max="4" value="2" step="1" />
</div>
<div class="bg-col">
<label class="bg-label">Height: <span id="bg-bh-val">80</span>px</label>
<input id="bg-bar-height" class="bg-range" type="range" min="50" max="200" value="80" step="5" />
</div>
</div>

<button class="bg-generate-btn" id="bg-generate-btn">Generate Barcode</button>
...
<button class="bg-dl-btn" id="bg-dl-btn">Download PNG</button>
</div>
<div class="bg-info-box" id="bg-info-box"></div>
</div>

<div class="bg-crosslinks">
<p>Generate QR codes &rarr; <a href="/tools/qr-code-generator/">QR Code Generator</a></p>
<p>Create invoices &rarr; <a href="/tools/invoice-generator/">Invoice Generator</a></p>
</div>
...
// narrow=1 wide=3, inter-char gap=1(space)
function expandC39(pattern, barW) {
var result = [];
for (var i = 0; i < pattern.length; i++) {
var w = pattern[i] === "1" ? 3 : 1;
result.push({ width: w * barW, dark: (i % 2 === 0) });
}
return result;
}
...
if (!C39_CHARS[chars[i]]) throw new Error("Code 39 supports A-Z, 0-9, and - . $ / + % space. Invalid: '" + chars[i] + "'");
}
return chars;
}
...
if (d.length !== 13) throw new Error("EAN-13 requires exactly 12 or 13 digits.");
var calc = ean13CheckDigit(d.substring(0, 12));
if (d[12] !== calc) throw new Error("EAN-13 check digit mismatch. Expected " + calc + ", got " + d[12] + ".");
return d;
}
...
if (i < chars.length - 1) totalBits += 1; // inter-char gap
}
var textH = 18;
...
if (i < chars.length - 1) x += barW; // gap
}
ctx.fillStyle = barColor;
ctx.font = "12px monospace";
ctx.textAlign = "center";
...
// Structure: quiet | start(3) | L*6(42) | middle(5) | R*6(42) | end(3) | quiet
var quietW = barW * 9;
var guardW = 3 * barW;
var midW   = 5 * barW;
var dataW  = 42 * barW;
...
// Set placeholder
var ph = { code128: "Any ASCII text (32–127)", code39: "A-Z 0-9 - . $ / + % space", ean13: "12 or 13 digits" };
dataEl.placeholder = ph[type] || "Enter data...";
}
...
infoBox.innerHTML = "<strong>Code 128B</strong> &mdash; Encoded " + data.length + " character(s). Checksum verified.";
} else if (type === "code39") {
var chars = encodeCode39(data);
renderCode39(canvas, chars, barColor, bgColor, barW, barH);
infoBox.innerHTML = "<strong>Code 39</strong> &mdash; " + (chars.length - 2) + " character(s) between start/stop asterisks.";
} else if (type === "ean13") {
var digits = encodeEAN13(data);
canvas._data = digits;
renderEAN13(canvas, digits, barColor, bgColor, barW, barH);
infoBox.innerHTML = "<strong>EAN-13</strong> &mdash; Check digit: <strong>" + digits[12] + "</strong>. Country prefix: <strong>" + digits.substring(0, 3) + "</strong>.";
}
