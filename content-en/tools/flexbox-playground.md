...
ShowToc: false
cover:
  image: "/images/covers/flexbox-playground.png"
  alt: "CSS Flexbox Playground"
---
...
<p class="fb-subtitle">Experiment with all Flexbox properties visually. Click an item to adjust its individual properties. Copy the generated CSS anytime.</p>
...
<div class="fb-layout">
  ...
  <div class="fb-right">
    <div class="fb-preview-wrap">
      <div class="fb-preview-header">Live Preview</div>
...
(function() {
  ...
  function renderTabs() {
    ...
    countLabel.textContent = state.itemCount + ' item' + (state.itemCount !== 1 ? 's' : '');
    itemLabel.textContent = '(Item ' + (state.selectedItem + 1) + ')';
  }
  ...
  fdEl.addEventListener('change', function() { state.container.flexDirection = this.value; renderPreview(); });
  fwEl.addEventListener('change', function() { state.container.flexWrap = this.value; renderPreview(); });
  jcEl.addEventListener('change', function() { state.container.justifyContent = this.value; renderPreview(); });
  aiEl.addEventListener('change', function() { state.container.alignItems = this.value; renderPreview(); });
  ...
  copyBtn.addEventListener('click', function() {
    ...
    if (navigator.clipboard) {
      navigator.clipboard.writeText(txt).then(function() {
        ...
        setTimeout(function() { copyBtn.textContent = 'Copy CSS'; copyBtn.classList.remove('copied'); }, 1800);
      });
    } else {
...
> CSS Clip-Path → [CSS Clip-Path Maker](/tools/css-clip-path/)
