---
title: "SVG Icon Maker - Create Custom SVG Icons"
date: 2025-05-16
...
<p class="subtitle">Draw custom SVG icons on a 24×24 grid — no software needed. Export clean SVG code instantly.</p>
...
<div class="layout">
  ...
  <div class="canvas-wrap">
    <div class="canvas-controls" id="canvas-controls">
      ...
      <label class="grid-toggle">
        <input type="checkbox" id="snap-grid" checked> Snap
      </label>
    </div>
    ...
    <div class="size-preview">
      ...
      <canvas id="prev-16" width="16" height="16" title="16px"></canvas> 16
      <canvas id="prev-32" width="32" height="32" title="32px"></canvas> 32
      <canvas id="prev-64" width="64" height="64" title="64px"></canvas> 64
    </div>
  ...
  <div>
    <div class="panel">
      ...
      <div class="output-box" id="svg-output">&lt;!-- Draw shapes to generate SVG --&gt;</div>
      <div class="btn-row">
        <button class="btn btn-primary btn-sm" id="copy-btn">Copy SVG</button>
        <button class="btn btn-secondary btn-sm" id="download-btn">Download .svg</button>
      </div>
      <div class="alert-ok" id="copy-ok">Copied to clipboard!</div>
...
(function(){
  ...
  const TOOLS = [
    ...
    {id:'line',    label:'Line'},
    {id:'path',    label:'Path (freehand)'},
  ];

  const PRESETS = [
    {label:'Arrow', d:'M4 12h16M13 5l7 7-7 7', name:'arrow'},
    {label:'Check', d:'M5 13l4 4L19 7', name:'check'},
    {label:'X',     d:'M18 6L6 18M6 6l12 12', name:'x'},
    {label:'Star',  d:'M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z', name:'star'},
    {label:'Heart', d:'M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z', name:'heart'},
  ];
  ...
  function addPreset(p) {
    ...
    shapes.push({id, type:'path', d:p.d, fill, stroke, strokeWidth:sw, opacity:op, label:`path (${p.label})`});
    renderShapes();
    selectShape(id);
    updateOutput();
    updateShapeList();
  ...
  function endDraw(e) {
    ...
    if (currentTool === 'circle') {
      ...
      shape = {id, type:'circle', cx:r(startPt.x), cy:r(startPt.y), r:r(rad), fill, stroke, strokeWidth:sw, opacity:op, label:`circle`};
      ...
      shape = {id, type:'rect', x, y, width:w, height:h, fill, stroke, strokeWidth:sw, opacity:op, label:`rect`};
    } else if (currentTool === 'line') {
      shape = {id, type:'line', x1:r(startPt.x), y1:r(startPt.y), x2:r(endPt.x), y2:r(endPt.y), fill:'none', stroke, strokeWidth:sw, opacity:op, label:`line`};
    } else if (currentTool === 'path') {
      if (pathPoints.length < 2) return;
      const d = pathPoints.map((p,i) => `${i===0?'M':'L'}${r(p.x)},${r(p.y)}`).join(' ');
      shape = {id, type:'path', d, fill:'none', stroke, strokeWidth:sw, opacity:op, label:`path`};
      pathPoints = [];
    }
  ...
  function renderShapes() {
    ...
    shapes.forEach(s => {
      ...
      if (el) {
        if (s.id === selectedId) {
          el.setAttribute('filter', 'drop-shadow(0 0 1px #6366f1)');
  ...
  function updateShapeList() {
    ...
    if (shapes.length === 0) {
      shapeListEl.innerHTML = '<div style="font-size:.78rem;color:#9ca3af;padding:6px">No shapes yet. Draw on the canvas.</div>';
      return;
    }
    shapes.forEach((s, i) => {
  ...
  document.getElementById('clear-btn').addEventListener('click', () => {
    if (!shapes.length || confirm('Clear all shapes?')) {
...
- [SVG Path Editor](/tools/svg-path-editor/) — Edit and visualize SVG path data
- [SVG to PNG Converter](/tools/svg-to-png/) — Convert SVG files to PNG images
- [Color Palette Generator](/tools/color-palette-generator/) — Generate harmonious color palettes
- [Favicon Generator](/tools/favicon-generator/) — Create favicons from scratch
- [Pixel Art Editor](/tools/pixel-art-editor/) — Draw pixel-perfect icons on a grid
